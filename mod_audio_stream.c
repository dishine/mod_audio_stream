...
static switch_status_t on_recv_dtmf(switch_core_session_t *session, const switch_dtmf_t *dtmf, switch_dtmf_direction_t direction)
{
    switch_channel_t *channel = switch_core_session_get_channel(session);
    private_t *tech_pvt = switch_channel_get_private(channel, "dtmf_private");

    if (dtmf) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
                          "Received DTMF [%c], duration: %d, direction: %d\n",
                          dtmf->digit, dtmf->duration, direction);

        char message[64];
        snprintf(message, sizeof(message), "{\"type\":\"dtmf\",\"digit\":\"%c\",\"duration\":\"%d\"}",
                 dtmf->digit, dtmf->duration);

        stream_session_send_text(session, message);
    }
...
static switch_bool_t capture_callback(switch_media_bug_t *bug, void *user_data, switch_abc_type_t type)
{
    ...
    switch (type) {
        ...
        case SWITCH_ABC_TYPE_CLOSE:
            {
                ...
                switch_core_event_hook_remove_recv_dtmf(session, on_recv_dtmf);
            }
            break;
        ...
        case SMBF_WRITE_STREAM:
            if (tech_pvt->close_requested) {
                return SWITCH_FALSE;
            }
            //switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "Got SWITCH_ABC_TYPE_WRITE_REPLACE.\n");
            //return stream_write_frame(bug);
            break;
        default:
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "mod_audio_stream: unhandled callback type: %d\n", type);
            break;
    }
                                     ...
                                     char* metadata)
{
    ...
    if (SWITCH_STATUS_FALSE == stream_session_init(session, responseHandler, read_codec->implementation->actual_samples_per_second,
                                                 wsUri, audioType, sampling, channels, metadata, &pUserData)) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "Error initializing mod_audio_stream session.\n");
        return SWITCH_STATUS_FALSE;
    }
    // 注册 DTMF 回调
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "adding hook dtmf.\n");
    switch_core_event_hook_add_recv_dtmf(session, on_recv_dtmf);
    ...
    if (zstr(cmd) || argc < 2 || (0 == strcmp(argv[1], "start") && argc < 4)) {
        ...
        if ((lsession = switch_core_session_locate(argv[0]))) {
            if (!strcasecmp(argv[1], "stop")) {
                ...
                strncpy(audioType, argv[3], sizeof(audioType) - 1);
                audioType[sizeof(audioType) - 1] = '\0'; // 确保字符串终止
                ...
                if (!validate_ws_uri(argv[2], &wsUri[0])) {
                    ...
                    status = start_capture(lsession, flags, wsUri, audioType, sampling, metadata);
                }
            } else {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
